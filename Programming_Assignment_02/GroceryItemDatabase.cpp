...
GroceryItemDatabase::GroceryItemDatabase( const std::string & filename )
{
  ...
  if (!fin.is_open()){ 
    ...
    while(!fin.eof()){
    fin >> std::quoted(upc) >> delimiter >> std::quoted(brand) >> delimiter >> 
          std::quoted(product) >> delimiter >> price;
    GroceryItem item(upc, brand, product, price);
...
GroceryItem * GroceryItemDatabase::find(const std::string &isbn){
  return find_recursive(isbn);
}
// GroceryItem * find_recursive(std::string & filename, const std::string &isbn){
//   GroceryItem * data;
//   std::string upcCode;
//   std::ifstream::getline( >> std::ws, upcCode, ', ') >> data; 
//   if(isbn == data->upcCode){
//     return data;
//   }
//   else{
//     if()
